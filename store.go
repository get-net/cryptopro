...
const (
	...
	CERT_CLOSE_STORE_CHECK_FLAG = C.CERT_CLOSE_STORE_FORCE_FLAG
)
...
func CertOpenSystemStore(storeName string) (*CertStore, error) {
	...
	if store == nil {
		return nil, fmt.Errorf("can`t get store %s", storeName)
	}
...
func CertCloseStore(store *CertStore, flags uint32) (C.int, error) {
	if store == nil {
		return -1, errors.New("store not exist")
	}
